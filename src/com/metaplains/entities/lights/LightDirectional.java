...
public class LightDirectional extends Light {
	...
	public LightDirectional(Scene scene) {
		super(scene);
	...
	public void updateDepthTextures(GameScreen screen) {
		...
		perspective(90F, 1, 0.1F, 1000.0F);
		setMatrixMode(VIEW);
		pushMatrix();
		lookAt(transform.position.clone().translate(0, 10, 0), transform.position.clone().translate(0, -1, 0), Vec3F.UNIT_X);
		setMatrixMode(MODEL);
		pushMatrix();
		glLoadIdentity();
